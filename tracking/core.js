...
!function( $ ) {
	...
	Datetimepicker.prototype = {
		...
		click: function(e) {
			...
			if (target.attr('class') == 'switch' && target.closest('div').hasClass('datetimepicker-days'))
				return;
			if (target.length == 1) {
    ...
    defaultParseErrors: function(errors, modal) {
        $.each( errors, function( key, value ) {
            ...
            if (el.is(':visible') && el.attr('type') != 'hidden') {
                ...
                el.closest('.form-group').append(error_block).addClass('has-error');
            }
...
var tables = {
    ...
    events: function(table) {
        ...
        $tableContainer.on('click', '[data-multi]', function () {
            ...
            data = tables.getFilters(table);
            data.id = tables.items_arr[table];
            data._method = 'DELETE';
            $.ajax({
                type: "POST",
                dataType: "json",
                url: tables.config[table][action],
                data: data,
                beforeSend: function() {
